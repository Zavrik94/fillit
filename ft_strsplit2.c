...
static int		countalf2(char *s, char ch, int i)
{
	int c;
...
static int		*ft_long3(char *s, char ch)
{
	int	i;
	int cword;
	int	c;
	...
	while (s[++i] != '\0')
	{
		if (s[i] == ch && s[i + 1] != ch)
		{
			res[c++] = countalf2(s, ch, i + 1);
		}
		else if (i == 0 && s[i] != ch)
		{
			res[c++] = countalf2(s, ch, 0);
		}
	}
	res[0] = cword;
	return (res);
...
static char		*ft_write2(char *s, int i, int c)
{
	char	*res;
	int		j;
...
char			**ft_strsplit2(const char *s)
{
	...
	while (s[++i])
	{
		c1 = c + 1;
		if (s[i] != ch && i == 0)
			rch[c++] = ft_write2((char *)s, i, res[c1]);
		else if ((s[i] != ch && s[i - 1] == ch))
			rch[c++] = ft_write2((char *)s, i, res[c1]);
	}
	rch[c] = 0;
